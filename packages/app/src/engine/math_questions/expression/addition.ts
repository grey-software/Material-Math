...
const DifficultyBoundsModel: DifficultyBounds = {
  basic: { min: 1, max: 10 },
  normal: { min: 5, max: 50 },
...
const getSubExprToken = (expr: Expr): SubExprToken => {
  const bounds: ExprBound = DifficultyBoundsModel[expr.diff]
  return {
    bounded: false,
...
const _getBoundedComplementTokens = (expr: BoundedExpr): BoundedComplementTokens => {
  ...
  const boundedRandom = randomNum(DifficultyBoundsModel[expr.diff].min, bound)
  return {
...
const _getRandomToken = (expr: Expr): ExprToken => {
  const randomValue = randomNum(DifficultyBoundsModel[expr.diff].min, DifficultyBoundsModel[expr.diff].max)
